...
- **桌宠首次亮窗钉在屏幕左上角（hot.21 旧默认值）**：`bounds` 默认 `{0,0}` 被当真实位置恢复——Windows 工作区原点 ≥ (8,8)，精确 (0,0) 只可能来自 hot.21 落盘的旧默认值。现改可空语义（null = 从未拖过，亮窗走默认右下位），读取时把 (0,0) 归一为 null，老数据自动修复；`smoke:pet-store` 补归一断言。
...
- **草稿确认页防误触（实战反馈"都选了填入，建出来只有名字"）**：确认页字段默认全选、单击即取消——与本应用其它选择器（可驱使 Agent、模型目录芯片）"单击选中"的肌肉记忆相反，逐个点"✓ 填入"实际是全部取消，填入表单后只剩无条件填入的名字。数据链路本身无 bug（主进程 parseAgents/normalize 往返冒烟 + 真实组件 jsdom 无头全流程点击对当前版与 v0.19.0 双版本实证字段无损）。修复为让取消状态四重可见：取消字段的值划线降透明、按钮文案变「已跳过」、页脚动态计数「将填入 N/6 个字段——按钮单击即取消/恢复」、全部取消时主按钮文案变「只填入名字」。验证：`gui-forge-test/`（esbuild 打包真实 AgentsView + jsdom 驱动的回归小架子，正常路径全字段通过 + 肌肉记忆路径四信号齐全；未提交）。
