...
侧栏「Issue」页是工作队列：标题 + 描述 + 工作目录 + 负责人，支持优先级/标签/截止日期/状态流转（backlog→todo→in_progress→in_review→done/blocked）。执行历史以 Run 记录在 Issue 时间线里，agent 跑完自动写报告评论；评论里 `@队员名` 会在同一 Issue 上触发新一轮执行。看板页按状态分列拖动卡片；每条 Issue 执行可展开实时日志、Markdown 结果、git diff 与追问。
...
- **自动化**：按分钟间隔定时用固定 prompt 唤醒某队员，产出可选落成 Issue 或只留执行日志（⚠️ 待定：功能未经完整设计，已知空工作目录兜底到临时目录、「仅执行」结果不可见、无运行历史等问题，暂不建议依赖）
...
└── renderer/              React 界面（Issue/看板/Agent/会议/目标/自动化/技能/用量/设置）
```
