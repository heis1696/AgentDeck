...
派单对账：已建子单在 Task 索引持久化父 runId 与派单幂等键；同一父运行重见相同派单时复用原子单。拒单原因也按父 runId 持久化，政策护栏与名单缺失均具名回灌；已建单随结果报告、拒单随反馈回合确认送达后才标记已送。领队追问或重启后续聊时，最多注入 30 条未确认结果并转义协议标记，避免把未建单误判为在途。回执为至少一次语义：后端已收到但本地确认写入前崩溃时可能再次提示；删除子任务会移除该子单回执，不能据此宣称跨删除恰好一次。

拒单身份：新回执以父 runId 和 `${to}\n${prompt}` 区分派单；同一次派单重复解析仍去重，同目标的不同指令即使拒因相同也各自留痕、回灌和确认送达。回执附带最多 60 字指令摘录，政策护栏分类只检查摘录前的拒因。旧记录没有派单 key 时保留原有拒因去重语义；重启迁移保留新 key，恢复对账按完整回执身份确认，避免相同摘录误确认其他派单。

事件写入恢复：运行回调按原有批次节奏写 `events.jsonl`，提交前先把本批次持久化到 `userData/tasks/<id>/pending-events/`（原子写入；访问权限沿用用户数据目录的系统 ACL）；成功后删除副本，启动时按稳定事件 ID 回放，避免不确定写盘结果导致重复。故障持续时终态和取消最多等待 5 秒，退出最多等待 2 秒；未落盘事件保留副本供重启回放，取消任务时明确提示日志未完整写入，删除任务时连同副本删除。磁盘连恢复副本也写不进去时只能明确报错，不能保证这些事件不丢失。
...
归池容量：同一进程内每仓最多保留 2 棵空闲树，容量检查、detach、元数据归池和登记整体受仓库级归池锁保护。锁顺序固定为 worktree 路径锁 → 仓库归池锁，持仓库锁时不再获取路径锁；取池只删除条目，不持仓库锁。容量已满或归池失败时走既有回收流程，不登记空占容量的条目，原有 owner 和 generation 核验继续生效。
