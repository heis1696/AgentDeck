...
> **写于** 2026-09（0.14.x–0.17.x 架构重构施工方案）｜ **被取代**：阶段 0–8 全部实施完成（验证记录见 docs/VALIDATION.md），编排/目标的后续施工由 ORCHESTRATION-GOAL-CONSTRUCTION.md（本目录）接棒 ｜ **现状看** docs/ARCHITECTURE.md。
...
> 编排与目标模式的最新施工顺序、学习结论、不变量和交互矩阵入口见 [`ORCHESTRATION-GOAL-CONSTRUCTION.md`](ORCHESTRATION-GOAL-CONSTRUCTION.md)。本文保留 0.14–0.18 的架构拆分历史，实施时以后者的阶段闸门为准。
...
3. 记录当前所有 `npm run smoke:*` 脚本的通过/失败矩阵，写入本文件或 `../VALIDATION.md`。
...
**验收**：`npm run typecheck`、`npm run smoke:all`、真实 `smoke:clis`、真实 `smoke:zcode` 和 `npm run dist` 通过；并发取消、启动期取消、429 resume/取消退避、显式 relay、流式提前委派和 Goal 重启恢复均有回归。真实 provider 的 429 与延迟观察写入 `reports/CONCURRENCY-HOTFIX-REPORT.md` 和 `../VALIDATION.md`。
...
6. 文档同步更新 `../ARCHITECTURE.md`、`../API.md` 和 `CHANGELOG.md`。
