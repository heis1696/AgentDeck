...
> 📦 **档案（2026-09-19 文档审计，基线 `6b2f038` / v0.22.0-hot.19）**：历史调研报告，已归档。
> **写于** 2026-09-09（方法论调研 + 六项目拆解计划 + 行动清单）｜ **被取代**：拆解已完成（本目录 teardown/ 七篇），行动项陆续落地（opencode server 化、goal 护栏、doom-loop 检测等） ｜ **现状看** docs/ARCHITECTURE.md 与本目录 ORCHESTRATION-GOAL-CONSTRUCTION.md。
...
> **状态：✅ 六项目拆解已完成**——报告见 `teardown/`（6 份，合计约 26 万字符，全部结论带文件路径:行号引用）；跨项目综合发现与 agentdeck 行动清单见 §5–6
...
本轮拆解按“先建立共同坐标系，再做源码精读，最后回写 agentdeck 决策”的顺序执行。`teardown/repos/` 只保存本地研究副本，研究产物只进入 `teardown/`，不把第三方源码纳入版本库。
...
| 4. 回归复核 | 检查文件名、链接、编码、`.gitignore`；对关键引用抽查行号和上下文，避免把研究建议误写成已实现功能 | `teardown/` 恰有六份目标报告；`teardown/repos/` 被忽略；总览状态与实际产物一致 |
...
六个项目浅克隆至 `teardown/repos/`（已 gitignore，不入库），6 个子代理并行拆解，各产出一份报告（沿用 `reports/MULTICA-TEARDOWN.md` 规格：一句话定位 / 总体架构 / 核心链路 / 关键实现细节 / **与 agentdeck 的对照与可借鉴点**；子代理各精读 20–46 个核心源码文件）。
