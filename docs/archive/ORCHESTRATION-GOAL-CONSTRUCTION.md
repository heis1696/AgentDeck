# Agent 编排与目标模式施工总册
> 📦 **档案（2026-09-19 文档审计，基线 `6b2f038` / v0.22.0-hot.19）**：历史施工总册，已归档。
> **写于** 2026-09-09（v1.0）｜ **被取代**：目标 v2、worktree 回收、opencode server 化、会议模式已分批落地（0.15.0–0.16.0），其余阶段未再按本册推进 ｜ **现状看** docs/ARCHITECTURE.md（§4 / §7.1）与 CHANGELOG。
...
六份 teardown 报告是设计输入，具体证据和文件行号见 `teardown/`。下表只保留会改变 AgentDeck 代码或验收方式的结论。
...
- `LOOP-ENGINEERING.md`：四层模型、agentdeck 映射、行动清单和拆解计划。
- `teardown/`：六份源码精读报告。
- 本文：不变量、风险台账、阶段顺序和验收协议。
- `reports/ARCHITECTURE-REVIEW.md`：现状风险登记册。
