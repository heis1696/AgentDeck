...
**npm 名与文件名不一致的映射（知识图谱建边时注意）：** `smoke`→`smoke-runner.mjs`、`smoke:clis`→`smoke-cli-adapters.mjs`、`smoke:worktrees`→`smoke-worktree-lifecycle.mjs`（链上还有 `smoke:worktree-ownership`/`smoke:worktree-timeout`）、`e2e:delegate`→`e2e-delegate-real.mjs`、`release:hot`→`release-hot.mjs`、`deploy:hot`→`deploy-feed.mjs`。
