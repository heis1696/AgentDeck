...
ok(by('t_old_leader').status === 'running', '无归属证明的旧 running 保持原状态')
ok(!by('t_old_leader').error, '没有死亡证据不写入中断错误')
...
console.log('普通任务：')
ok(by('t_running').status === 'running' && !by('t_running').error, '启动读取不擅自结束其他实例或身份未知的运行')
ok(by('t_ok').status === 'done' && by('t_ok').result === 'r', 'done 任务原样保留')

console.log('落盘：')
ok(Array.isArray(JSON.parse(fs.readFileSync(path.join(tasksDir, 'tasks.json'), 'utf8'))), '只读加载不自动迁移磁盘数据')
store.update('t_ok', { title: '显式更新' })
