...
if (adoptedState.tasks.find((task) => task.id === 't-orphan')?.status !== 'queued') throw new Error('orphan takeover did not transition the durable Task to queued')
if (manager.snapshot?.orphanRuns.length !== 0) throw new Error('orphan takeover left stale manager orphan state')
const secondTakeover = await manager.recoverOrphans()
if (secondTakeover.adopted.length !== 0) throw new Error('active orphan lease was not enforced')
