// 锻造师：agents:draft / agents:improve 的主进程实现——按一句描述生成队员草稿，或按反馈改进既有队员
// 结构仿 skills.ts：纯函数（内置技能正文/落盘/解析/拼 prompt）不 import electron、路径由参数注入，便于 smoke；
// runForgeTurn 是唯一的主进程编排入口（ctx 注入 backends/presets/settings/sharedDir），draft/improve 共用
import fs from 'node:fs'
import path from 'node:path'
import { app } from 'electron'
import { FORGE_SKILL_NAME, FORGE_SKILL_VERSION, isForgeAgent, type AgentDraft, type DraftResult, type ImproveResult } from '../shared/forge'
...
const FORGE_SKILL_DESCRIPTION = '把一句队员描述扩写成 AgentDeck 队员定义草稿，或按反馈最小改动地修订既有队员定义（JSON 输出）'
...
/** v2 内置正文：生成（含澄清追问）+ 改进双模式；用户可在共享目录编辑同名技能覆盖它 */
...
export function ensureForgeSkill(sharedDir: string): void {
  ...
  try {
    ...
    // 已是当前版本，或正文与 v1 内置不同（用户编辑过）→ 尊重现状；只有未经编辑的旧版才升级
    if (Number(data.version) >= FORGE_SKILL_VERSION) return
    if (body.trim() !== FORGE_SKILL_BODY_V1.trim()) return
  } catch {}
