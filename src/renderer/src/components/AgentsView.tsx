...
/** 支持连接注入的后端面：zcode（runtimeModel）/ claude（spawn env）——只作 Agent 表单预设选择器的显示门槛；预设本身是全局连接档案，不归属平台 */
 ...
 *  预设是全局连接档案（不归属平台），零全局切换——agent 按 id 引用预设后，
  ...
  const presetSection = (
    <section className="tm-section">
      <div className="tm-section-head">
        ...
        <span className="tm-section-desc">全局连接档案（baseURL / 密钥 / 线协议），不归属平台；Agent 选取后在它的会话里生效，不改全局配置。zcode / claude 支持注入执行。</span>
      </div>
      ...
      {editing && (
        <div className="overlay" ref={editingLayerRef} onClick={(e) => e.target === e.currentTarget && setEditing(null)}>
          <div className="dialog">
            ...
            {CONNECTION_BACKENDS.includes(editing.backend) && (
              <label className="field">
