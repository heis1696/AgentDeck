...
export function useInteractionLayer<T extends HTMLElement = HTMLElement>(options: InteractionLayerOptions<T>): RefObject<T> {
  ...
  useLayoutEffect(() => {
    ...
    const zIndex = interactionLayers.zIndexOf(layerId)
    applyLayerZIndex(layerRef.current, zIndex)
    ...
    return () => {
      ...
      if (layerRef.current && zIndex > 0 && layerRef.current.style.zIndex === String(zIndex)) layerRef.current.style.zIndex = ''
