...
export function validatePetManifest(value: unknown): PetManifest | null {
  ...
  // 包级渲染提示（可选）：pixelated=最近邻放大（像素包缺省），smooth=浏览器高质量缩放（AI/高清包）
  const rendering = raw.rendering
  if (rendering !== undefined && rendering !== 'pixelated' && rendering !== 'smooth') return null
  return {
